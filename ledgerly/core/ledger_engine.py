...
hooks.py. For each save, the engine looks up active Ledger Configs for this
DocType (cached), computes the tracked value (single field or child-table
sum) and compares against the pre-save value, and if changed, creates and
submits a Ledger Entry with computed delta, balance, and populated dimension
columns.

Failures inside the engine are logged and swallowed so a misconfigured
ledger can never break a basic source-doc save.
"""
...
def _should_capture(doc, method):
    """Reject hook events that can't usefully produce a ledger entry."""
...
def _get_active_configs_for_doctype(source_doctype):
    """Return Ledger Config names whose source matches the given DocType. Cached."""
    cache = frappe.cache()
