...
_FEEDER_CACHE_KEY = "custom_ledger:type2_feeders"
_EMPTY = "__none__"
...
def capture_submit(doc, method=None):
    ...
    try:
        ...
        frappe.log_error(
            title=f"Custom Ledger transactional engine: submit failed for {doc.doctype} {doc.name}",
...
def permit_feeder_cancel(doc, method=None):
    ...
    Custom Ledger reverses those entries in ``capture_cancel`` (on_cancel), so the
...
def capture_cancel(doc, method=None):
    ...
    try:
        ...
        frappe.log_error(
            title=f"Custom Ledger transactional engine: cancel failed for {doc.doctype} {doc.name}",
            message=frappe.get_traceback(),
        )
...
def _process_submit(doc) -> None:
    ...
    for source in sources:
        if source["child_table_field"]:
            frappe.log_error(
                title="Custom Ledger: skipping child-table source (not yet supported)",
                message=(
...
def _create_entry_for_source(doc, source: dict) -> None:
    ...
    if not carrier_name:
        frappe.log_error(
            title="Custom Ledger: missing carrier link value",
            message=(
...
def _write_carrier_balance(
    *, carrier_doctype: str, carrier_name: str, balance_field: str, ledger_config: str
    ...
    frappe.flags.custom_ledger_engine_writing = True
    try:
        frappe.db.set_value(
            carrier_doctype, carrier_name, balance_field, total, update_modified=False
        )
    finally:
        frappe.flags.custom_ledger_engine_writing = False
